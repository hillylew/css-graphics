(function () {
    ...
    const moduleSpacingPercentage = 1.5; // 2% of container width
    const arraySpacingPercentage = 3; // 5% of container width
    ...
    /* ----------------------- Draw Sun ----------------------- */
    const sunGroup = svg.append("g").attr("transform", "translate(10, 10)");
    ...
    const triangleWidth = sunRadius * 0.1; // Width of the base of the triangles' triangles
    ...
    for (let i = 0; i < numberOfTriangles; i++) {
        ...
        const triangleEndX = sunCenterX + (sunRadius + gap + triangleLength) * Math.cos(angle);
        const triangleEndY = sunCenterY + (sunRadius + gap + triangleLength) * Math.sin(angle);
    
        const triangle = [
            { x: triangleStartX, y: triangleStartY },
            { x: triangleStartX + triangleWidth * Math.cos(angle - Math.PI / 2), y: triangleStartY + triangleWidth * Math.sin(angle - Math.PI / 2) },
            { x: triangleEndX, y: triangleEndY },
            { x: triangleStartX + triangleWidth * Math.cos(angle + Math.PI / 2), y: triangleStartY + triangleWidth * Math.sin(angle + Math.PI / 2) }
        ];
    
        sunGroup.append("path")
            .attr("d", `M${triangle[0].x},${triangle[0].y} L${triangle[1].x},${triangle[1].y} L${triangle[2].x},${triangle[2].y} L${triangle[3].x},${triangle[3].y} Z`)
            .attr("fill", "#ffcb03");
    }
    ...
    const drawOctagonCell = (group, x, y, width, height) => {
        ...
        group.append("path").attr("d", pathData).attr("fill", "#3167a4")
            .attr("class", "cell")
            .on("mouseover", function () {
    ...
    const drawPanel = (panelGroup) => {
        for (let r = 0; r < panelRows; r++) {
            for (let c = 0; c < panelColumns; c++) {
                ...
                drawBorder(moduleGroup, totalModuleWidth, totalModuleHeight, cellToBorderGap, "module");
        ...
        drawBorder(panelGroup, totalPanelWidth, totalPanelHeight, moduleSpacing, "panel");
    };
  
    
    // Calculate the total area occupied by the array
    const arrayWidth = arrayColumns * totalPanelWidth + (arrayColumns - 1) * arraySpacing;
    const arrayHeight = arrayRows * totalPanelHeight + (arrayRows - 1) * arraySpacing;

    const drawArray = (arrayGroup) => {
        
        // Draw a white rectangle as background for the array
        arrayGroup.insert("rect", ":first-child") // Insert the rectangle as the first child of the group
